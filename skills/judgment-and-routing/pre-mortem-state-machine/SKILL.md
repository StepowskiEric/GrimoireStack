...
Write at least five specific narratives from the retrospective vantage point. Specific means "the data migration took three weeks instead of five days because the legacy schema had undocumented nullability constraints" — not "scope creep." Generate before ranking; do not filter for plausibility while writing. Cover multiple categories: execution, dependency, assumption, scope/complexity, human/coordination, timing, unknown unknowns. Five minimum; eight to ten for high-stakes plans. Then trace second-order consequences for the top two: what does each failure cause next, and who inherits that?
