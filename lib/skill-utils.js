...
const AGENT_CONFIGS = {
  ...
  omp: { defaultDest: path.join(os.homedir(), '.omp', 'agent', 'skills'), flat: true },
  factory: { defaultDest: path.join(os.homedir(), '.factory', 'skills'), flat: true },
};
...
function getSkillFiles(dir, base) {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    } else if (entry.name.endsWith('.md') && entry.name !== 'README.md' && entry.name !== 'RESEARCH.md') {
