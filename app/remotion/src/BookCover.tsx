...
function _randomRunes(count: number): string {
  let s = '';
...
const NoiseOverlay: React.FC = () => (
  <AbsoluteFill
    style={{
      opacity: 0.03,
      background: 'repeating-conic-gradient(#000 0%, transparent 0.1%, #000 0.2%)',
    }}
  />
);
...
const BookBackground: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  return (
    <AbsoluteFill style={{ opacity }}>
      <AbsoluteFill
        style={{ background: `radial-gradient(ellipse at 50% 40%, ${PARCHMENT}, ${DARK})` }}
      />
    </AbsoluteFill>
  );
};
...
const WaxSeal: React.FC<{ frame: number }> = ({ frame }) => {
  ...
  const glow = interpolate(frame, [80, 120, 200, 250], [0, 0.4, 0.4, 0], {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            style={{ fontSize: 44, color: GOLD_GLOW, textShadow: '0 0 10px rgba(212,175,55,0.3)' }}
          >
            ⛧
          </span>
        </div>
...
const CandleGlow: React.FC<{ frame: number }> = ({ frame }) => {
  const flicker =
    0.7 + Math.sin(frame * 0.3) * 0.15 + Math.sin(frame * 0.7) * 0.1 + Math.sin(frame * 1.1) * 0.05;
