...
let pageCreakAudio: HTMLAudioElement | null = null;
let ambienceStarted = false;
const ambienceNodes: (AudioScheduledSourceNode | GainNode)[] = [];
...
// ambience / whisper scheduler stops. The React side (App.tsx) owns
...
function stopAmbience() {
  ...
  for (const node of ambienceNodes) {
    try {
      if (typeof (node as { stop?: () => void }).stop === 'function') (node as { stop: () => void }).stop();
