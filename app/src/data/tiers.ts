export interface SpellTierInput {
  status?: string;
  combos?: string[];
}

export interface SpellTierMeta {
  label: string;
  className: string;
  title: string;
}

export type SpellTier = 'faded' | 'apprentice' | 'adept' | 'master' | 'archmage';

export function getSpellTier(spell: SpellTierInput): SpellTier {
...
export const TIER_META: Record<SpellTier, SpellTierMeta> = {
  faded: {
    label: 'Faded Glyph',
