...
export default function CompareSpellsModal({
  left: leftProp,
  right: rightProp,
  onClose,
  onSelect,
  ...
  const [pickerSlot, setPickerSlot] = useState<'left' | 'right' | null>(null); // 'left' | 'right' | null
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-[600px] rounded-sm border border-border bg-surface p-6"
        ref={modalRef}
        role="dialog"
        aria-modal="true"
        aria-label="Compare spells"
      >
        <button
        ...
        {!pickerSlot ? (
            ...
            <div className="mt-4 flex items-center gap-3">
              <div className="flex-1">
                ...
                >
                  {left ? (
                    <>
                      <div className="text-[1.3rem] text-[rgba(212,175,55,0.7)]">
                        <SchoolSigil schoolId={leftSchool?.id} size={32} />
                      </div>
                      <div className="mt-1 font-['Cormorant_Garamond'] text-[0.78rem] text-[#e8dcc4]">
                        {left.name}
                      </div>
                      <div className="mt-1 font-['Special_Elite'] text-[0.5rem] text-[rgba(168,152,120,0.4)]">
                        〈 {left.skill} 〉
                      </div>
                      <div className="mt-1 text-[0.55rem] text-[#5a4a3a]">Click to replace</div>
                    </>
                  ) : (
                    <div className="font-['Cormorant_Garamond'] italic text-[0.78rem] text-[#6a5a3a]">
                      Summon the first incantation
                    </div>
                  )}
                </button>
              </div>
              <div
                className="font-['Cinzel'] shrink-0 text-[0.7rem] text-[#6a5a3a]"
                aria-hidden="true"
              >
                vs
              </div>
              <div className="flex-1">
                ...
                >
                  {right ? (
                    <>
                      <div className="text-[1.3rem] text-[rgba(212,175,55,0.7)]">
                        <SchoolSigil schoolId={rightSchool?.id} size={32} />
                      </div>
                      <div className="mt-1 font-['Cormorant_Garamond'] text-[0.78rem] text-[#e8dcc4]">
                        {right.name}
                      </div>
            ...
            {left && right ? (
              <div className="mt-4">
                ...
                {comparison.map((row) => (
                  <div
                    key={row.key}
                    data-testid="compare-table-row"
                    data-state={row.same ? 'same' : 'diff'}
                    className={cn(
                      'relative grid grid-cols-[1fr_70px_1fr] gap-2 border-b border-[rgba(180,140,80,0.06)] px-2.5 py-2 text-[0.68rem]',
                      row.same ? '' : 'bg-[rgba(212,175,55,0.03)]',
                    )}
                  >
                    <div className="text-left text-[#d8ccb5]">
                      <span className="text-[0.65rem]">{row.left || <em>—</em>}</span>
                    </div>
                    <div className="text-center text-[0.55rem] uppercase tracking-widest text-[#6a5a3a]">
                      {row.label}
                    </div>
                    <div className="text-right text-[#d8ccb5]">
                      <span className="text-[0.65rem]">{row.right || <em>—</em>}</span>
                    </div>
                    {!row.same ? (
                      <span
                        className="absolute right-1 top-1/2 -translate-y-1/2 text-[0.5rem] text-gold"
                        aria-hidden="true"
                      >
                        ●
                      </span>
              ...
              <div className="mt-4 text-center text-[0.72rem] text-[#6a5a3a]">
                Bind two incantations to weigh them. The Eye will set them side by side: effect,
                status, and the threads that bind them.
              </div>
            )}
          </>
        ) : (
          <div className="mt-4 flex flex-col gap-2">
            <div className="flex items-center justify-between text-[0.72rem] text-[#a89878]">
              ...
              <button
                type="button"
                className="text-[0.65rem] text-[#6a5a3a] transition-colors hover:text-gold"
                onClick={() => {
                  setPickerSlot(null);
                  setPickerQuery('');
                }}
            ...
            <div className="max-h-[300px] overflow-y-auto">
              {filtered.length === 0 ? (
                <div className="py-5 text-center font-['Cormorant_Garamond'] italic text-[0.78rem] text-[#5a4a3a]">
                  The abyss returns no incantations for this scrying.
                </div>
              ) : (
                filtered.map(({ spell, school }) => {
                  const isThis =
                    (pickerSlot === 'left' && left?.skill === spell.skill) ||
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "flex w-full items-center gap-2 rounded border px-3 py-2.5 text-left font-['Cormorant_Garamond'] text-[#d8ccb5] transition-colors min-h-11",
                        isThis
                          ? 'border-[rgba(180,140,80,0.08)] opacity-40 cursor-default'
                          : 'border-[rgba(180,140,80,0.08)] bg-[rgba(26,20,10,0.4)] hover:border-border-hover hover:bg-[rgba(42,32,18,0.6)]',
                      )}
                      onClick={() => pickSpell(spell, school)}
                      disabled={isThis}
                    >
                      <span
                        className="text-[0.8rem] text-[rgba(212,175,55,0.7)]"
                        aria-hidden="true"
                      >
                        <SchoolSigil schoolId={school.id} size={20} />
                      </span>
                      <span className="text-[0.72rem] text-[#e8dcc4]">{spell.name}</span>
                      <span className="font-['Special_Elite'] text-[0.5rem] text-[rgba(168,152,120,0.4)]">
                        〈 {spell.skill} 〉
                      </span>
                    </button>
                  );
