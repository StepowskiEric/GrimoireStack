...
export default function GrimoireStackLayout({
  ...
  const renderContent = () => {
      ...
      return (
        <div className="flex flex-col gap-4">
          <div className="relative border border-[rgba(138,154,106,0.32)] bg-[rgba(8,10,16,0.7)] p-4 rounded-sm">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(138,154,106,0.1),transparent_55%),radial-gradient(circle_at_70%_80%,rgba(100,70,20,0.08),transparent_55%)] pointer-events-none" aria-hidden="true" />
            <div className="relative flex items-center gap-4">
              <span className="text-sickly opacity-90 flex-shrink-0 relative">
                <span className="absolute -inset-1 rounded-full border border-[rgba(212,175,55,0.35)]" aria-hidden="true" />
                <span className="absolute -inset-2 rounded-full border border-dashed border-[rgba(138,154,106,0.18)]" aria-hidden="true" />
                <SchoolSigil schoolId={activeSchool.id} size={32} />
              </span>
              <div className="min-w-0">
                <h2 className="font-['Cinzel_Decorative'] font-black text-[1.2rem] text-gold-bright tracking-wider leading-tight" style={{ textShadow: '0 0 18px rgba(212,175,55,0.3), 0 2px 0 rgba(0,0,0,0.7)' }}>{activeSchool.real}</h2>
                <p className="font-['Cinzel'] text-[0.6rem] uppercase tracking-[0.2em] text-silver-dim mt-0.5">{activeSchool.spells.length} {activeSchool.spells.length === 1 ? 'spell' : 'spells'} bound within this school</p>
              </div>
              <div className="ml-auto flex-shrink-0">
                <button
                  ...
                  onClick={() => previousTab ? handleTabSelect(previousTab) : handleTabSelect(TABS.ABOUT)}
                  aria-label="Back"
                >
                  ← Back
                </button>
    ...
    switch (activeTab) {
      ...
      case TABS.CHANGELOG:
        return (
          <ChangelogView onSpellClick={handleSpellCardClick} />
        );
      case TABS.SETTINGS:
        return (
          <SettingsView
  ...
  return (
    ...
    >
        ...
        <aside className="w-full min-w-full flex-row flex-wrap gap-2 p-3 mb-3 md:w-60 md:min-w-60 md:flex-col md:p-4 bg-[rgba(8,10,16,0.6)] border border-[rgba(138,154,106,0.08)] rounded-lg backdrop-blur-md" aria-label="Sidebar">
          ...
          <div className="w-full mt-2 pt-2 border-t border-[rgba(138,154,106,0.08)] flex flex-col gap-2 md:mt-auto md:pt-3">
            ...
            <div className="flex flex-col gap-2">
              <button className="w-full inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 border border-border text-text-muted text-[0.68rem] uppercase tracking-wider transition-colors hover:border-border-hover hover:text-text-primary" onClick={onShowShortcuts} type="button">
                <Icon name="sigil" size={14} /> Shortcuts
              </button>
              <button className="w-full inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 border border-border text-text-muted text-[0.68rem] uppercase tracking-wider transition-colors hover:border-border-hover hover:text-text-primary" onClick={onExportJson} type="button">
                <Icon name="archive" size={14} /> Export JSON
              </button>
              <button className="w-full inline-flex items-center justify-center gap-1.5 px-2.5 py-1.5 border border-border text-text-muted text-[0.68rem] uppercase tracking-wider transition-colors hover:border-border-hover hover:text-text-primary" onClick={onExportMarkdown} type="button">
      ...
      {webOpen && (
        <div className="fixed inset-0 z-[200] bg-[rgba(5,5,8,0.97)] flex flex-col" role="dialog" aria-modal="true" aria-label="Spell Web">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(138,154,106,0.15)]">
            <span className="font-['Cinzel'] text-sm font-semibold uppercase tracking-widest text-sickly">Spell Web</span>
            <button
              type="button"
