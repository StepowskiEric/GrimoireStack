...
export default function SpellCard({ spell, onClick, matched, children, isFavorited, onToggleFavorite }) {
  ...
  return (
    <div
      ...
      className={`relative overflow-hidden rounded-sm border border-[rgba(100,70,30,0.15)] bg-gradient-to-br from-[#e0d4bc] to-[#d4c8b0] p-4 transition-all duration-200 ${matched === true ? 'glow border-[rgba(212,175,55,0.25)] shadow-[0_0_20px_rgba(212,175,55,0.08),0_0_40px_rgba(212,175,55,0.04),inset_0_0_30px_rgba(212,175,55,0.03)]' : ''} ${matched === false ? 'dim opacity-35 grayscale-[80%] pointer-events-none' : 'hover:border-[rgba(212,175,55,0.18)] hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.3),0_0_15px_rgba(212,175,55,0.04)] active:scale-[0.98]'}${hasTrueName ? ' has-true-name' : ''}`}
    ...
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_25%_20%,rgba(212,175,55,0.03),transparent_55%)] opacity-0 transition-opacity duration-200 group-hover:opacity-100 pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-2 right-2.5 text-[0.5rem] text-[rgba(212,175,55,0.04)] pointer-events-none" aria-hidden="true">✦</div>
      <div className="mb-1 flex items-center justify-between">
        <div data-testid="spell-tier">
          {showTierBadge ? (
            <span className={`inline-flex items-center gap-1 rounded-full border border-[rgba(180,140,80,0.35)] bg-[rgba(35,22,12,0.55)] px-2.5 py-1 font-['Cinzel'] text-[0.58rem] uppercase tracking-wider ${tierInfo.className}`} title={tierInfo.title}>
              <span className="text-[0.7rem] text-gold-bright" aria-hidden="true">⟐</span>
              <span>{tierInfo.label}</span>
            </span>
          ) : (
            <span className="font-['Cinzel'] text-[0.55rem] uppercase tracking-wider text-[#3e2712]">{legacyTier || 'common'}</span>
          )}
        </div>
        {onToggleFavorite && (
          <button
      ...
      {hasTrueName ? (
        <>
          <div className="spell-true-name font-['Cormorant_Garamond'] text-[1.18rem] font-semibold leading-tight text-[#130b02]" style={{ textShadow: '0 0 8px rgba(212,175,55,0.08)' }}>{spell.trueName}</div>
          <div data-testid="spell-name--secondary" className="spell-name--secondary font-['Cinzel'] text-[0.72rem] font-normal italic text-[#5a4428] mt-0.5 lowercase tracking-wider">{spell.name}</div>
        </>
      ) : (
        <div className="spell-name font-['Cinzel'] text-[1rem] font-semibold leading-tight text-[#130b02]">{spell.name}</div>
      )}
      <div className="font-['Special_Elite'] text-[0.7rem] text-[#241708] my-0.5">〈 {spell.skill} 〉</div>
      <div className="text-[0.88rem] leading-snug text-[#1f1508] [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical] overflow-hidden">{spell.effect}</div>
