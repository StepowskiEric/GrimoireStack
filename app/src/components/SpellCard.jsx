...
export default function SpellCard({
  ...
  return (
    ...
    >
      <div
        className="absolute inset-0 bg-[radial-gradient(circle_at_30%_20%,rgba(180,140,60,0.08),transparent_50%),radial-gradient(circle_at_70%_80%,rgba(120,80,20,0.06),transparent_50%)] opacity-60 pointer-events-none"
        aria-hidden="true"
      />
      <div
        className="absolute inset-0 bg-[url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22><filter id=%22n%22><feTurbulence type=%22fractalNoise%22 baseFrequency=%220.8%22 numOctaves=%223%22 stitchTiles=%22stitch%22/></filter><rect width=%22200%22 height=%22200%22 filter=%22url(%23n)%22 opacity=%220.04%22/></svg>')] opacity-80 mix-blend-multiply pointer-events-none"
        aria-hidden="true"
      />
      <div
        className="absolute bottom-2 right-2.5 text-[0.5rem] text-[rgba(160,120,60,0.12)] pointer-events-none"
        aria-hidden="true"
      >
        ✦
      </div>
      <div className="mb-1 flex items-center justify-between">
        <div data-testid="spell-tier">
          {showTierBadge ? (
            <span
              className={`inline-flex items-center gap-1 rounded-full border border-[rgba(140,110,60,0.4)] bg-[rgba(60,40,15,0.35)] px-2.5 py-1 font-['Cinzel'] text-[0.58rem] uppercase tracking-wider ${tierInfo.className}`}
              title={tierInfo.title}
            >
              <span className="text-[0.7rem] text-[#c8a86c]" aria-hidden="true">
                ⟐
              </span>
              <span>{tierInfo.label}</span>
            </span>
          ) : (
            <span className="font-['Cinzel'] text-[0.55rem] uppercase tracking-wider text-[#6b4a2a]">
              {legacyTier || 'common'}
            </span>
          )}
        </div>
      ...
      <div className="text-[0.88rem] leading-snug text-[#4a3020] [display:-webkit-box] [-webkit-line-clamp:2] [-webkit-box-orient:vertical] overflow-hidden">
        {spell.effect}
      </div>
      <div className="mt-2 flex items-center justify-between">
        <span
          className={`font-['Cinzel'] rounded-md border px-2 py-0.5 text-[0.5rem] uppercase tracking-wider ${statusClass === 'proven' ? 'text-[#c8e8c8] border-[rgba(106,170,106,0.55)] bg-[#1a2e1a]' : statusClass === 'new' ? 'text-[#f0dcb0] border-[rgba(201,168,76,0.55)] bg-[#2e2210]' : statusClass === 'framework' ? 'text-[#d8cce8] border-[rgba(138,122,170,0.55)] bg-[#1e1828]' : statusClass === 'hybrid' ? 'text-[#c8e4ec] border-[rgba(122,184,201,0.55)] bg-[#122428]' : statusClass === 'includes' ? 'text-[#ecc8c8] border-[rgba(201,122,122,0.55)] bg-[#2a1414]' : 'text-[#3e2712] border-[rgba(100,70,30,0.35)] bg-[rgba(35,22,12,0.55)]'}`}
        >
          {legacyTier || 'common'}
        </span>
        <span className="font-['Cormorant_Garamond'] italic text-[0.7rem] text-[#2a1a0a] transition-colors duration-200 group-hover:text-[#5a4428]">
