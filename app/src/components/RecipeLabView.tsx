...
import type { SpellEntry } from '../data/spellCore.ts';
import { cn } from '../utils/cn.ts';
import SchoolSigil from './SchoolSigil.tsx';

export default function RecipeLabView({ onCompareOpen, onCompareTwo }: { onCompareOpen?: any; onCompareTwo?: any }) {
  ...
  return (
    <div className="py-1">
      <h2
        className="font-['Cinzel_Decorative'] text-[1.25rem] font-bold text-center text-moonlight tracking-wide mt-1 mb-1"
      ...
      <p className="font-['Cormorant_Garamond'] italic text-center text-[0.82rem] text-silver-mute mb-4.5">
        Select two incantations to summon a side-by-side comparison. The Eye will weigh which is
        fitter for your need.
      </p>
      ...
      <div className="mb-5.5 p-3.5 border border-[rgba(138,154,106,0.22)] rounded-sm bg-gradient-to-b from-[rgba(8,12,18,0.7)] to-[rgba(4,6,10,0.85)] relative">
        <div
          className="absolute top-0 left-3.5 right-3.5 h-px bg-gradient-to-r from-transparent via-[rgba(196,71,71,0.5)] to-transparent"
          aria-hidden="true"
        />
          ...
          <div className="flex flex-wrap gap-2 mb-3.5">
            {selectedSpells.map((item) => (
              <div
                ...
                className="inline-flex items-center gap-2 px-2.5 py-1.5 bg-[rgba(2,2,4,0.5)] border border-[rgba(138,154,106,0.2)] rounded-sm"
              >
                <span className="text-[1.1rem]">
                  <SchoolSigil schoolId={item.school.id} size={20} />
                </span>
                <span className="font-['Cinzel'] text-[0.68rem] font-semibold tracking-wide text-moonlight">
                  {item.spell.name}
                </span>
                <button
      ...
      <div>
          ...
          {visible.map((item) => {
            ...
            return (
              <button
                key={item.spell.skill}
                className={cn(
                  'flex flex-col items-center p-2.5 text-center border rounded-sm cursor-pointer transition-all duration-200',
                  selected
                    ? 'bg-gradient-to-b from-[rgba(40,32,8,0.55)] to-[rgba(28,20,6,0.7)] border-[rgba(212,175,55,0.5)] shadow-[0_0_10px_rgba(212,175,55,0.15)]'
                    : 'bg-[rgba(2,2,4,0.5)] border-[rgba(138,154,106,0.15)] hover:bg-[rgba(20,28,40,0.6)] hover:border-[rgba(196,71,71,0.4)] hover:-translate-y-0.5 hover:shadow-[0_0_8px_rgba(196,71,71,0.1)]',
                )}
                onClick={() => handleSpellSelect(item)}
                type="button"
              ...
              >
                <span className="text-[1.4rem] mb-1 drop-shadow-[0_1px_2px_rgba(0,0,0,0.5)]">
                  <SchoolSigil schoolId={item.school.id} size={20} />
                </span>
                <span className="font-['Cormorant_Garamond'] text-[0.75rem] text-moonlight leading-snug">
                  {item.spell.name}
                </span>
                <span className="font-['Cinzel'] text-[0.6rem] uppercase tracking-widest text-silver-mute mt-1 text-center">
                  {item.school.real}
                </span>
              </button>
            );
        ...
        {pageCount > 1 ? (
          <nav
            className="flex items-center justify-center gap-3.5 mt-3 pt-2 border-t border-dashed border-[rgba(122,58,90,0.18)]"
            aria-label="Ritual incantation pagination"
          >
            <button
              type="button"
            ...
            <span className="font-['Cinzel'] text-[0.74rem] text-silver-mute tracking-widest">
