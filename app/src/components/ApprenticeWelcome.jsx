...
export default function ApprenticeWelcome({ onClose }) {
  ...
  return (
    ...
    >
      <div
        className={cn(
          'relative w-full max-w-[520px] rounded-sm border border-border bg-surface p-8 text-center',
          visible && 'animate-modalIn',
        )}
        ref={modalRef}
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div
          className={cn('flex flex-col items-center gap-5', visible && 'animate-panelIn')}
          key={current.titleKey + index}
        >
          <div className="text-[2.4rem] text-gold text-shadow-gold">
            <ModalEye size={36} />
          </div>
          <h2 className="font-['Cinzel'] text-[clamp(1.1rem,2.5vw,1.45rem)] font-bold leading-snug tracking-wide text-[#6a3a1a]">
            {t(current.titleKey)}
          </h2>
          <p className="text-[0.98rem] leading-relaxed text-[#2a1a0a]">{t(current.bodyKey)}</p>
          <p className="font-['Cormorant_Garamond'] italic text-[0.88rem] leading-relaxed text-[#5a4428]">
            {t(current.accentKey)}
          </p>
        </div>
        ...
        <div className="mt-5 flex items-center justify-center gap-2" aria-hidden="true">
          {PANELS.map((_, i) => (
            <div
              key={i}
              className={cn(
                'h-[3px] flex-1 rounded-[3px] bg-[rgba(100,70,30,0.15)] transition-colors duration-300',
                i === index && 'bg-gold',
                i < index && 'bg-[rgba(212,175,55,0.45)]',
              )}
            />
          ))}
        </div>
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            {index > 0 ? (
              <button
                className="font-['Cinzel'] text-[0.58rem] uppercase tracking-wider text-silver transition-colors hover:border-border-hover hover:text-gold"
