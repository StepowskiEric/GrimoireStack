...
import { getNewlyAdded, getRecentlyUpdated, getUpdated, type ChangeFeedItem } from '../data/changeFeed.ts';

function ChangelogEntry({ item, onSpellClick }: { item: ChangeFeedItem; onSpellClick?: any }) {
  const handleClick = () => {
    if (onSpellClick) {
...
function groupByDate(items: ChangeFeedItem[]) {
  const groups: Record<string, ChangeFeedItem[]> = { today: [], thisWeek: [], thisMonth: [], older: [] };
...
export default function ChangelogView({ onSpellClick }: { onSpellClick?: any }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (schoolFilter !== 'all') {
      items = items.filter((item: ChangeFeedItem) => item.school.id === schoolFilter);
    }

    if (dateRange.from) {
      items = items.filter((item: ChangeFeedItem) => (item.lastUpdated || '') >= dateRange.from);
    }
    if (dateRange.to) {
      items = items.filter((item: ChangeFeedItem) => (item.lastUpdated || '') <= dateRange.to);
    }
