import { SEANCE_MAX_QUESTIONS } from '../../data/consultationData.ts';
...
export default function InsightMeter({ insight, max = SEANCE_MAX_QUESTIONS }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="h-1.5 flex-1 bg-surface-overlay rounded-full overflow-hidden"
        aria-hidden="true"
      >
        <div className="h-full bg-accent" style={{ width: `${Math.max(0, ratio) * 100}%` }} />
      </div>
