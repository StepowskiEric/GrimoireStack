...
import { useConsultation } from '../hooks/useConsultation.ts';
import { cn } from '../utils/cn.ts';
import InsightMeter from './consultation/InsightMeter.tsx';
import QuestionCard from './consultation/QuestionCard.tsx';
import ResultCard from './consultation/ResultCard.tsx';
import SanityMeter from './consultation/SanityMeter.tsx';
import SigilPicker from './consultation/SigilPicker.tsx';
import SigilSvg from './consultation/SigilSvg.tsx';
import TentacleSvg from './consultation/TentacleSvg.tsx';
 ...
 */
export default function CommuneView({ onSpellClick, audioEnabled = false }: { onSpellClick?: any; audioEnabled?: boolean }) {
  const {
    stage,
    sanity,
    insight,
  ...
  useEffect(() => {
    ...
    if (sanity <= 3 && !whispersStartedRef.current) {
      ...
      import('../audio/sounds.ts').then((mod) => mod.startWhispers?.());
    }
  }, [sanity, audioEnabled]);
