...
_Skills that actually help find and fix code bugs._
...
| Failing unit tests — need to determine if test or code is wrong | [`unit-test-debugging`](../development/unit-test-debugging/SKILL.md)                      | Zoom-out-first workflow: diagnose whether test or code is source of truth, prevents fix-loops | New |
| Writing Jest tests or need Jest syntax reference       | [`jest-testing`](../development/jest-testing/SKILL.md)                                               | Matchers, async patterns, mocking, config, React Native testing                          | New |
...
_Skills for structured thinking when the problem is unclear._
...
| Multiple possible causes, competing explanations              | [`occams-razor`](../judgment-and-routing/occams-razor/SKILL.md)                                                      | List alternatives ranked simplest → most complex; test simplest first; every added assumption must earn its keep                              |
...
| Multiple competing explanations that all fit the evidence     | [`occam-abduction`](../judgment-and-routing/occam-abduction/SKILL.md)                                                | Generate hypotheses → audit evidence → rank by assumption count → falsify or confirm the simplest                                             |
...
| Multiple strategy branches, need to allocate effort wisely    | [`occam-mcts`](../orchestration/occam-mcts/SKILL.md)                                                                 | Occam's Razor ranks branches by complexity first; MCTS explores simplest viable branches before complex ones                                  |
...
_Skills that let the agent automatically discover solutions that outperform human intuition._
...
_Skills for improving systems, workflows, or outputs over time._
...
| Before any significant task — prevent confidently doing the wrong thing | [`pre-flight-intent-verification`](../orchestration/pre-flight-intent-verification/SKILL.md)  | Clarify → Specify → Ground gate: surfaces critical assumptions, crystallizes intent into testable scenarios, verifies facts before acting |
...
| Learn from what actually happened (after)                 | [`retrospective`](../execution/retrospective/SKILL.md)                                                  | After-action review with Five Whys, action items                                                                          |
...
| Stress-test a proposal with a separate adversarial agent | [`advocatus-diaboli`](../judgment-and-routing/advocatus-diaboli/SKILL.md)                     | Separate sub-agent attacks your proposal before committing |
...
_Skills for verifying reasoning, preventing hallucinations, and managing token burn in long-horizon tasks._
...
| Equip a sub-agent with the right skills for a task | [`subagent-composer`](../orchestration/subagent-composer/SKILL.md)                                                             | Task-to-skill mapping; loads TDD + relevant skills automatically                             |
...
_Skills for measuring and improving other skills._
